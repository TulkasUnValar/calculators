...
def calculate_integral():
    ...
    rightmost_point_answer.delete(0, tk.END)
    leftmost_point_answer.delete(0, tk.END)
    middle_point_answer.delete(0, tk.END)
    ...
    try:
        ...
        # Calcular la integral por rectángulos con altura en extremo izquierdo
        left_result = 0
        for i in range(n):
            xi = a + i * delta_x
            left_result += f_x.subs(x, xi) * delta_x

        # Calcular la integral por rectángulos con altura en extremo derecho
        right_result = 0
        for i in range(1, n + 1):
            xi = a + i * delta_x
            right_result += f_x.subs(x, xi) * delta_x

        # Calcular la integral por rectángulos con altura en punto medio
        middle_point_result = 0
        for i in range(n):
            xi = a + (i + 0.5) * delta_x
            middle_point_result += f_x.subs(x, xi) * delta_x

        # Mostrar los resultados en la interfaz gráfica
        leftmost_point_answer.insert(tk.INSERT, string=left_result)
        rightmost_point_answer.insert(tk.INSERT, string=right_result)
        middle_point_answer.insert(tk.INSERT, string=middle_point_result)
        
        # Crear una gráfica de la función en el intervalo dado
...
# --------------------------------------------------------FrontEnd------------------------------------------------------
# Crear ventana principal, instancia ventana
window = tk.Tk()
window.title("Integration by Rectangle")
...
# Imprime en ventana "Leftmost point"
leftmost_point_label = tk.Label(answer_frame, text="Leftmost point")
leftmost_point_label.grid(row=2, column=0, padx=10, pady=5)
# Respuesta Leftmost point
leftmost_point_answer = tk.Entry(answer_frame, width=30)
leftmost_point_answer.grid(row=2, column=1, padx=10, pady=5)

# Imprime en ventana "Rightmost point"
rightmost_point_label = tk.Label(answer_frame, text="Rightmost point")
rightmost_point_label.grid(row=1, column=0, padx=10, pady=5)
# Respuesta Rightmost point
rightmost_point_answer = tk.Entry(answer_frame, width=30)
rightmost_point_answer.grid(row=1, column=1, padx=10, pady=5)
           
# Imprime en ventana "Middle point"
middle_point_label = tk.Label(answer_frame, text="Middle point")
middle_point_label.grid(row=3, column=0, padx=10, pady=5)
# Respuesta Hexadecimal
middle_point_answer = tk.Entry(answer_frame, width=30)
middle_point_answer.grid(row=3, column=1, padx=10, pady=5)
